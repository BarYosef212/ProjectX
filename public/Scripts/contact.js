...
async function fetchBranchesAndAddMarkers(map) {
  try {
    // Fetch branches data from the backend (adjust URL)
    ...
    for (const branch of data.branches) {
      const { location, branchName } = branch;
      ...
      if (location && location.coordinate_x && location.coordinate_y) {
        ...
        // Handle marker click for redirection (replace with your actual route)
        marker.addListener("click", () => {
          window.location.href = `/branches/${branch.id}`; // Update route based on your backend
        });
      } else {
        console.warn(`Branch '${branchName}' does not have valid coordinates.`);
      }
