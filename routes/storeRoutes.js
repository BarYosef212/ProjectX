...
//     const shoes = await shoesController.getShoes();
...
router.get('/store', async (req, res) => {
  try {
    ...
    let shoes = await shoesController.getShoes({ priceFilter, skip, limit });
    ...
    res.render('shoesStore', { shoes, currentPage: page, totalPages, priceFilter });
  } catch (error) {
    console.log(error);
